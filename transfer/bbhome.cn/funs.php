...
function export_cat(){	
		...
		//迁移商品分类数据
		$sql = "insert into sdb_mall_offer_pcat (offerid,pid,catid,cat,catord,catiffb) select 1,parentid,id,classname,orderid,1  from shop_productclass";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
...
function export_brand(){
	...
	$sql = "insert into sdb_mall_brand (brand_id,offerid,sbid,brand_name) select id,1,0,brandname from tmp_brand group by id";
	mysql_query($sql) or  err(__LINE__." ".mysql_error());
}
...
function export_goods(){
		...
		//导商品数据
		$sql = "Insert into sdb_mall_goods(gid,offerid,catid,bn,brand_id,goods,storage,priceintro,basicprice,price,danwei,intro,memo,smallimgremote,bigimgremote,onsale,uptime,weight) select id,1,class,productcode,brand,productname,storagenum,price,shopprice,shopprice,unit,summary,description,smallpic,middlepic,1,unix_timestamp(addtime),weight from shop_product group by id order by addtime DESC ";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
...
function export_users() {
		...
		//修改性格字段为文本型
		$sql = "ALTER TABLE `sdb_mall_member` CHANGE `sex` `sex` VARCHAR( 4 ) NOT NULL DEFAULT '1'"; 
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
		
		//处理用户
		$sql = "Insert into sdb_mall_member(userid,offerid,user,password,name,birthday,regtime,email,addr,mov,tel,zip,ip,point,point_history,pw_question,pw_answer,level,babyname,expected_childbirth,sex,babybirthday,mobile2,officephone,email2,often_contact,other_contact) select id,1,username,userpwd,mathername,matherbirthday,unix_timestamp(regtime),useremail, consigneeaddr,ConsigneeMobile,babytel,ConsigneePostalcode,loginip,userpoint,userpoint,userpwdquestion,useranswer,'1',babyname,babyycq,babysex,babybirthday,babymobile,babyofficetel,babymail,babyctype,babyCtypeOther from shop_userinfo group by id order by regtime DESC";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());

		//更新所有的男性为1
		$sql = "update sdb_mall_member set sex='1' where sex='男'";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
		//更新所有的女性为2
		$sql = "update sdb_mall_member set sex='2' where sex='女'";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());

		//将性格字段改成数字
		$sql = " ALTER TABLE `sdb_mall_member` CHANGE `sex` `sex` TINYINT( 1 ) NOT NULL DEFAULT '1'"; 
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
