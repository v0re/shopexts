...
<?php
	...
	while($row=mysql_fetch_array($rs))
	{
		...
		$small_gif = str_replace(".jpg",".gif",$small);
		$big_gif = str_replace(".jpg",".gif",$big);

		if(!file_exists($small)){
			$gif_flag = true;
			$small = $small_gif;
			$big = $big_gif;
			$news=$prefix."small/".$row['gid'].".gif";
			$newb=$prefix."big/".$row['gid'].".gif";
		}else{
		...
		{
				if($gif_flag){
					$query="update sdb_mall_goods set smallimgremote='".$row['gid'].".gif' where gid='".$row['gid']."'";
				}else{
		...
		{
				if($gif_flag){
					$query="update sdb_mall_goods set bigimgremote='".$row['gid'].".gif' where gid='".$row['gid']."'";
				}else{
