...
function export_cat(){	
		...
		mysql_query($sql) or  err(__LINE__." ".mysql_error());		
		//迁移商品分类数据
		$sql = "insert into sdb_mall_offer_pcat (catid,offerid,pid,cat,catord,catiffb,meta_keywords,categorydesc) select categoryid,1,parentid,category,categoryorder,1,meta,intro  from category  ";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
		//统一处理catpath
...
function export_goods(){
		...
		//导商品数据
		$sql = "Insert into sdb_mall_goods(gid,offerid,catid,bn,goods,brand,storage,priceintro,price,danwei,intro,memo,smallimgremote,bigimgremote,onsale,uptime,meta_description,meta_keywords) select id,1,categoryid,grade,name,mark,stock,price1,price2,type,introduce,detail,pic,pic,1,unix_timestamp(adddate),metawords,metaintro from product order by adddate DESC";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
...
function export_users() {
		...
		//导入会员等级
		$sql = "insert into sdb_mall_member_level (offerid,levelid,name,point) select 1,grpid,grpname,minipost from timesgrp";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
		
		//处理用户
		$sql = "Insert into sdb_mall_member(userid,offerid,user,password,name,regtime,email,oicq,sex,addr,mov,tel,zip,city,ip,point,point_history,pw_question,pw_answer,level,advance) select userid,1,username,userpassword,realname,unix_timestamp(adddate),useremail,userqq, usersex,address,usermobile,usertel,postcode,city,img,score,score,userquestion,useranswer,GRPid,deposit  from user  order by adddate DESC";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
...
function export_article(){		
		...
		//转移文章
		$sql = "Insert into sdb_mall_offer_ncon(newsid,offerid,catid,title,con,uptime,ifpub) select newsid,1,7,newsname,newscontent,unix_timestamp(adddate),1 from news order by adddate DESC";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
}
