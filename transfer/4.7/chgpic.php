<?php
	...
	$query="select gid,smallimgremote,bigimgremote from mall_goods";
	...
	{
		//print_r($row);
		$small=$prefix."small/".$row['smallimgremote'];
		$big=$prefix."big/".$row['bigimgremote'];
		$news=$prefix."small/".$row['gid'].".jpg";
		$newb=$prefix."big/".$row['gid'].".jpg";
		if(@rename($small,$news))
		{
				$query="update mall_goods set smallimgremote='".$row['gid'].".jpg' where gid='".$row['gid']."'";
				//print $query;
		...
		if(@rename($big,$newb))
		{
				$query="update mall_goods set bigimgremote='".$row['gid'].".jpg' where gid='".$row['gid']."'";
				//print $query;
				mysql_query($query) or die(mysql_error());
