...
class site_ctl_admin_explorer_app extends site_admin_controller 
{
    ...
    public function directory() 
    {
        ...
        if(!$this->check($app_id, $content_path))
            $this->_redirect('index.php?app=site&ctl=admin_explorer_app&act=index');  //验证是否可以浏览
        
        $fileObj = kernel::single('site_explorer_file');
        $dir = realpath(APP_DIR . '/' . $app_id . '/' . str_replace('-', '/', $content_path) . '/' . str_replace(array('-','.'), array('/','/'), $open_path));   //open_path不允许有'./'&'../'
        $filter=array(
                 'id' => $app_id,
                 'dir' => $dir,
                 'show_bak' => false,
