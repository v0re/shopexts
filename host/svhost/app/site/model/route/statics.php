...
class site_mdl_route_statics extends dbeav_model 
{
    ...
    public function insert(&$data) 
    {
        ...
        if($res){
            kernel::single('site_route_static')->set_dispath($data['static'], $data);
    ...
    public function update($data, $filter, $mustUpdate = null) 
    {
        ...
        if($res){
            foreach($old_rows AS $row){
                kernel::single('site_route_static')->del_dispath($row['static']);
                kernel::single('site_route_static')->del_genurl($row['url']);
            }
            $rows = $this->getList('*', $filter);
            foreach($rows AS $row){
                kernel::single('site_route_static')->set_dispath($row['static'], $row);
