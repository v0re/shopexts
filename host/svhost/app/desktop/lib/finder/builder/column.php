           ...
           $in_use = array_flip($this->getColumns());
            ...
            foreach($all_columns as $key=>$col){
                ...
                $html .= '<div class="row-line item"><input type="hidden" value="'.$key.'" name="allcol[]" />'.$mv_handler.'<input type="checkbox" '.(isset($in_use[$key])?' checked="checked" ':'').' value="'.$key.'" name="col[]" id="finder-col-set-'.$i.'" />
                    <label for="finder-col-set-'.$i.'">'.$col['label'].'</label></div>';
                $html .= '</div>';
            }
            $finder_id=$_GET['_finder']['finder_id'];   
            $html .= '<!-----.mainHead-----&darr;&nbsp;拖动改变顺序-----.mainHead----->';
            $html .= '<!-----.mainFoot-----<div class="table-action"><button class="btn btn-primary" onclick="$(\''.$domid.'\').fireEvent(\'submit\',{stop:$empty})"><span><span>保存提交</span></span></button></div>-----.mainFoot----->';
            ...
            <script>
              (function(){
                new Sortables($('{$domid}'),{clone:true,opacity:.7,handle:'.move-handler',onStart:function(){
