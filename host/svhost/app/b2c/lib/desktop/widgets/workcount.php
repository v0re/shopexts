...
class b2c_desktop_widgets_workcount implements desktop_interface_widget{
    ...
    function get_html(){
        ...
        $new_leave_message = $member_comments->count($filter);
        ...
        $filter = array('store|lthan'=>$alert_num);
        $mdl_goods = $this->app->model('goods');        
        $alert_num_count = $mdl_goods->count($filter);
        $render->pagedata['alert_num_count'] = intval($alert_num_count);
        $render->pagedata['alert_num'] = intval($alert_num);

        //到货通知
        $member_goods = $this->app->model('member_goods');
        $goods_notice = $member_goods->count(array('type'=>'sto','status'=>'send'));
