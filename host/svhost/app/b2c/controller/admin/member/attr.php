...
class b2c_ctl_admin_member_attr extends desktop_controller{
    ...
    function remove($attr_id){
        $this->begin('index.php?app=b2c&ctl=admin_member_attr&act=index');
        $attr_model = &$this->app->model('member_attr');
        $this->end($attr_model->delete($attr_id),__('选项删除成功'));
    }
    
    function show_switch($attr_id){
        $this->begin('index.php?app=b2c&ctl=admin_member_attr&act=index');
        $attr_model = &$this->app->model('member_attr');
        $this->end( $attr_model->set_visibility($attr_id,true),__('已设置显示状态'));
    }
    
    function hidden_switch($attr_id){
        $this->begin('index.php?app=b2c&ctl=admin_member_attr&act=index');
        $attr_model = &$this->app->model('member_attr');
        $this->end( $attr_model->set_visibility($attr_id,false),__('已设置关闭状态'));
    }
    
    function save_order(){
        $this->begin('index.php?app=b2c&ctl=admin_member_attr&act=index');
        $attr_model = &$this->app->model('member_attr');
        $this->end( $attr_model->update_order($_POST['attr_order']),__('选项排序更改成功'));
    }
}
