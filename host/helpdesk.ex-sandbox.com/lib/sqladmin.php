...
//										die;
								...
								case "1":
									//hey, the guy wants to download a  image
...
//									die;
			...
			if (!is_array($fields)) {
				...
				if ($_POST["returnurl"]) {
					...
					//check to see if there is a storeredirect
					if (($_GET["storeredirect"] == "ADDDETAILS") && $was_added ) {
						header("Location:" . "index.php?mod={$_GET[mod]}&sub={$_GET[sub]}&action={$this->forms[uridata][details]}&{$this->forms[table_uid]}={$_POST[$this->forms[table_uid]]}&returnurl={$_POST[returnurl]}");
					} else {				
