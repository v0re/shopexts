...
/*
	...
	$Id: index.php 718 2010-01-24 16:17:18Z anjel $
*/
...
#all{width:984px;margin:0 auto;}
	...
	.nocom .tabs ul li{float:left;height:23px;border-top:1px solid #e9e9f5;list-style:none;padding:0 10px;background:url(http://top.baidu.com/static/img/split1.gif) left top no-repeat;}
	...
	.com .tabs ul li{float:left;height:23px;border-top:1px solid #eaecf3;list-style:none;padding:0 10px;background:url(http://top.baidu.com/static/img/split1.gif) left top no-repeat;}
	...
	.com_hover img{background:url(http://top.baidu.com/static/img/icon.gif) left top;}
	.com_hover .com_on img{background:url(http://top.baidu.com/static/img/icon.gif) left -12px;}
...
<input type="submit" value="<?php echo $config["name"];?>一下" />
</form>
</div>
    </div>
	<div id="m">
                           ...
                           <div class="tc">
                             <table width="100%" border="0" cellspacing="0" cellpadding="0">
                               <tr>
                                <?php
								$j=0;
								$key_query=$db->query("select * from kuaso_search_keyword where s='".$cate["cate_id"]."' and char_length(keyword)<6 order by hits desc limit 20");
                              ...
                              <div class="tc">
                                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                  ...
                                  <tbody id="listdata">
                                    <?php
	$j=0;
	$key_query=$db->query("select * from kuaso_search_keyword where char_length(keyword)<6 and keyword regexp '[^0-9]+$' and keyword regexp '[^a-zA-Z]+$' order by hits limit 25");//^[^a-zA-Z]+$
	while($key=$db->fetch_array($key_query))
	{$j++;
...
<?php
	$j=0;
	$key_query=$db->query("select * from kuaso_search_keyword where s='".$s."' and char_length(keyword)<6 order by hits limit 50");
	while($key=$db->fetch_array($key_query))
	{$j++;
?>
  <tr>
...
?>  </tbody>
	...
	<div class="clear"></div>
	<div id="footer"><?php echo "<div style=\"text-align:center;margin-top:3px;\">&copy;2009 Powered by <a target=\"_blank\" href=\"http://www.kuaso.com/\">kuaso</a></div>";?></div>
</div>
</center>
