...
/*
	...
	$Id: search.class.php 2010-01-24 16:17:18Z anjel $
*/
...
class search
{
   ...
   function q($wd,$domain='')
   {
	  ...
	  // $ordersql=rtrim($ordersql,"+");
	   ...
	   else
	   {
		      ...
		      {
		          $sql="select * from kuaso_links where title<>'' and url like '%.".$domain."%' or url like '%//".$domain."%'"; //echo $sql;
		      }
	          else
		      {
		           $sql="select * from kuaso_links where title<>'' and url regexp 'http://".$domain."'";//echo $sql;
		       }
	   }
	   //echo $sql;
	   $query=$db->query($sql);
   ...
   function GetTg()
   {
		...
		$sql="select a.*,b.* from kuaso_zz_links a left join kuaso_zz_user b on a.user_id=b.user_id where b.points>=a.price and a.user_id>0 and ".$keywordsql." order by price desc limit 10";//echo $sql;
	    $query=$db->query($sql);
	    while($row=$db->fetch_array($query))
         {
		      $data["title"]=$this->GetRedKeyWord(str_cut($row["title"],60));
