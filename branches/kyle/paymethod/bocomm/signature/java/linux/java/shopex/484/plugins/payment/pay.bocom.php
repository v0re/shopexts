...
class pay_bocom extends paymentPlugin{
    ...
    function toSubmit($payment){
			...
			$realpath = HOME_DIR."/upload/bocom/".$payment['M_payment']."/";
			...
			$aREQ["goodsURL"]=$this->callbackUrl;
			//自动跳转时间,非必填
			$aREQ["jumpSeconds"]=0;
			...
			$merSignMsg = $this->bocomm_sign($realpath.md5('bocomshopex'.$payment['M_payment']).".xml",$srcMsg);
    ...
    function callback($in,&$paymentId,&$money,&$message,&$tradeno){
			...
			$realPath=HOME_DIR."/upload/bocom/".$tmp['payment']."/";
			$signMsg = $returnmsg[count($returnmsg)-1];

			$srcMsg="";
			for($i=0;$i<count($returnmsg)-1;$i++)
			{
				$srcMsg.=$returnmsg[$i]."|";
			}

			$ret = -100;
			$ret = $this->bocomm_verify($realPath.md5('bocomshopex'.$tmp['payment']).".xml",$srcMsg,$signMsg);
	...
	function createxml($aaData)
	{
		...
		$realPath=HOME_DIR."/upload/bocom/".$aaData['pay_id']."/";
