...
<html>
    ...
    <body bgcolor = "#FFFFFF" text = "#000000">
        <?php
            ...
            $begDate="";                        //开始时间[yyyyMMddHHmmss]
            $endDate="";                        //结束时间[yyyyMMddHHmmss]
            $flag=0;                            //订单状态 0 所有 1 已支付 2 已撤销 3 部分退货 4退货处理中 5 全部退货 
            $begIndex=0;                        //起始索引号，可为NULL,默认填0
            $begOrder="";                       //起始订单号，可为NULL
            $endOrder="";                       //截至订单号，可为NULL
            $sortField=1;                       //排序字段1，订单号；2，金额；3，时间
            ...
            //发主机交易
            $rep=$client->queryHistoryOrder($begDate, $endDate, $flag, $begIndex, $begOrder, $endOrder, $sortField,
                                            $sortOrder); //批量订单查询
            ...
            if ($code == "0") //交易成功
                ...
                $total=$opr->getValueByName("totalNumber");            //得到返回记录个数
