            ...
            case "NPS":
                ...
                break;
					...
					case "CCB":  
         				...
         				$return['MERCHANTID'] = $this->merid;
								$return['POSID'] = $this->ikey;
								$return['BRANCHID'] = '110000000';
								$return['ORDERID'] = $this->orderno.$this->orderid;
								$return['PAYMENT'] = number_format($this->amount,2,".","");
								$return['CURCODE'] = '01';
								$return['TXCODE'] = '520100';
								$return['REMARK1'] = '';
								$return['REMARK2'] = '';
								...
								$return['MAC'] = md5($signstr);

                //$hiddenString = "<input type=hidden name=shopex_encoding value=\"gb2312\" />";
								foreach($return as $name=>$value){
									$hiddenString .= "<input type=hidden name=\"{$name}\" value=\"{$value}\" />";
								}

                $this->action = "https://ibsbjstar.ccb.com.cn/app/ccbMain";
                return $hiddenString;
                break;
...
//                $this->action = "http://124.42.2.165/gateway/transForward.jsp";
            ...
            case "ABANK": //联银通支付平台-收汇宝
                ...
                if($this->currency == 1) {$lang = 1;}
                else if($this->currency == 2) {$lang = 2;$this->currency = 15;}
                /*
                $hiddenString.= "<input type=hidden name=\"BillNo\" value=\"065113\">";    //订单编号  ".$this->orderno."
                $hiddenString.= "<input type=hidden name=\"MerNo\"  value=\"1000\">";    //商户编号  ".$this->merid."
                $hiddenString.= "<input type=hidden name=\"Amount\" value=\"0.01\">"; //订单金额  ".$this->amount."
                $hiddenString.= "<input type=hidden name=\"Currency\" value=\"1\">";    //支付卡种  ".$this->currency."
                $hiddenString.= "<input type=hidden name=\"ReturnURL\" value=\"http://127.0.0.1/PayTest/PayResult.php\">";    //支付结果返回的商户URL   ".$ret_url."
                $hiddenString.= "<input type=hidden name=\"Language\" value=\"1\">";    //语言 ".$lang."
                $hiddenString.= "<input type=hidden name=\"Remark\" value=\"php test\">";    //商户数据包 -- 备注 ".$this->rnote."
                $md5src = $this->merid.$this->orderno.$this->currency.$this->amount.$lang.$ret_url.$this->ikey;
                $hiddenString.= "<input type=hidden name=\"MD5info\" value=\"9BB30E92DBDB670FD7DD61FDAC269714\">";    //MD5签名验证 ".strtoupper(md5($md5src))."
                */
                ...
                $hiddenString.= "<input type=hidden name=\"Language\" value=\"".$lang."\">";    //语言 
                $hiddenString.= "<input type=hidden name=\"Remark\" value=\"".$this->rnote."\">";    //商户数据包 -- 备注 
                //$md5src = $this->merid.$this->orderno.$this->currency.$this->amount.$lang.$ret_url.$this->ikey;
                //Change $this->orderno to $this->orderid for ctopay submit address change
                $md5src = $this->merid.$this->orderid.$this->currency.$this->amount.$lang.$ret_url.$this->ikey;
                $hiddenString.= "<input type=hidden name=\"MD5info\" value=\"".strtoupper(md5($md5src))."\">";    //MD5签名验证 
                
                $this->action = "https://payment.ctopay.com/payment/Interface";
                return $hiddenString;
                break;
            case "HYL":
                ...
                break;
						...
						case "ICBC_INST":
								$tmp_url = $this->url."index.php?gOo=icbc_inst_reply.do";
                $this->rnote = $this->dolocal($this->rnote,"zh");
           			#商户公钥文件
                 ...
                 *///////////////////////////////////////////////////////////////////////  								 
									...
									//分期付款期数
									$aREQ["installmentTimes"] =  $_COOKIE['icbc_period'][$this->orderno] ? $_COOKIE['icbc_period'][$this->orderno] : 1;
									//
									$aREQ["merURL"] = $tmp_url;
									//HS方式实时发送通知；AG方式不发送通知；
									$aREQ["notifyType"] = "HS";
									...
									//xml
									$tranData = "<?xml version=\"1.0\" encoding=\"GBK\" standalone=\"no\"?><B2CReq><interfaceName>".$aREQ["interfaceName"]."</interfaceName><interfaceVersion>".$aREQ["interfaceVersion"]."</interfaceVersion><orderInfo><orderDate>".$aREQ["orderDate"]."</orderDate><curType>".$aREQ["curType"]."</curType><merID>".$aREQ["merID"]."</merID><subOrderInfoList><subOrderInfo><orderid>".$aREQ["orderid"]."</orderid><amount>".$aREQ["amount"]."</amount><installmentTimes>".$aREQ["installmentTimes"]."</installmentTimes><merAcct>".$aREQ["merAcct"]."</merAcct><goodsID>".$aREQ["goodsID"]."</goodsID><goodsName>".$aREQ["goodsName"]."</goodsName><goodsNum>".$aREQ["goodsNum"]."</goodsNum><carriageAmt>".$aREQ["carriageAmt"]."</carriageAmt></subOrderInfo></subOrderInfoList></orderInfo><custom><verifyJoinFlag>".$aREQ["verifyJoinFlag"]."</verifyJoinFlag><Language>".$aREQ["Language"]."</Language></custom><message><merHint>".$aREQ["merHint"]."</merHint><remark1>".$aREQ["remark1"]."</remark1><remark2>".$aREQ["remark2"]."</remark2><merURL>".$aREQ["merURL"]."</merURL><merVAR>".$aREQ["merVAR"]."</merVAR></message></B2CReq>";
								 ...
								 */
								 ...
								 $this->action = "https://mybank.icbc.com.cn/servlet/ICBCINBSEBusinessServlet";
								 
								 $aFinalReq['interfaceName'] = $aREQ["interfaceName"];
								 $aFinalReq['interfaceVersion'] = $aREQ["interfaceVersion"];
								 $aFinalReq['tranData'] = $tranData;
								 $aFinalReq['merSignMsg'] = $merSignMsg;
								 $aFinalReq['merCert'] = $merCert;
