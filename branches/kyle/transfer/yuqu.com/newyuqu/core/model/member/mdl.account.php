...
class mdl_account extends shopObject {
     ...
     /*
        ...
        **/
        ...
        if($len<3){
            ...
            $message = '用户名过长!';
            return false;
        }elseif(!preg_match('/^[a-z0-9\@\-\_\.\x{4e00}-\x{9fa5}\x{f900}-\x{fa2d}\x{3041}-\x{3093}\x{30a1}-\x{30f6}\x{ac00}-\x{d7a3}]+$/iu', $uname)){
            $message = '用户名包含非法字符!';
            return false;
        }else{
        ...
        if(!$passport){
            ...
            if($row = $this->db->selectrow($sql)){
                ...
                return    $row;
								...
								//兼容ASP的16位md5密码
								$_16bit_passwd = substr(md5($passwd),8,16);
								$sql = 'select member_id,member_lv_id,email,uname,b_year,b_month,b_day,password,unreadmsg,cur,lang,point from sdb_members where uname='.$this->db->quote($login).' and password='.$this->db->quote($_16bit_passwd)." and disabled='false'";
								if($row = $this->db->selectrow($sql)){
									$row['secstr'] = $this->cookieValue($row['member_id']);
									$oMsg = $this->system->loadModel('resources/msgbox');
									$row['unreadmsg'] = $oMsg->getNewMessageNum($row['member_id']);
									//修正passwod
									$sql = 'update sdb_members set password='.$this->db->quote(md5($passwd)).' where member_id=\''.$row['member_id'].'\'';
									$this->db->exec($sql);
									return    $row;
								}else{
									return false;
    ...
    */
    ...
    function createUserFromPluin($data,&$message,$uid){
        if ($data['passwd_r']){
            if($data['passwd']!=$data['passwd_r']){
                $message = '两次密码输入不一致！';
                return false;
    ...
    function PlugUserRegist($userdb='',$memberid='',$username='',$password=''){
        if (is_array($userdb)){
