...
function export_cat(){	
		...
		mysql_query($sql) or  err(__LINE__." ".mysql_error());		
		//迁移商品数据
		$sql = "insert into sdb_mall_offer_pcat (catid,offerid,pid,cat,catiffb) select cat_id,1,0,cat_name,1  from shop_book_cate ";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
		//统一处理catpath
...
function export_brand(){
	...
	mysql_query($sql) or  err(__LINE__." ".mysql_error());
	$sql = "insert into sdb_mall_brand (offerid,sbid,brand_name) select 1,0,book_mo  FROM shop_books GROUP BY book_mo";
	mysql_query($sql) or  err(__LINE__." ".mysql_error());
}
...
function export_goods(){
		...
		mysql_query($sql) or  err(__LINE__." ".mysql_error());	
		//导商品数据
		$sql = "Insert into sdb_mall_goods(gid,offerid,catid,goods,brand_id,storage,priceintro,price,danwei,intro,memo,smallimgremote,bigimgremote,onsale,uptime,brand) select id,1,book_cate,book_name,0,book_visible,book_newpr,book_mempr,booksize_na,book_draw ,concat(book_descri,'<br>',book_change),concat('upload/0/',id,'_1.jpg'),concat('upload/0/',id,'_1.jpg'),book_show,unix_timestamp(book_time),book_mo from shop_books order by book_time DESC";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
		
		$sql = "select brand_id,brand_name from sdb_mall_brand";
		$rs = mysql_query($sql) or  err(__LINE__." ".mysql_error());
		while($row = mysql_fetch_array($rs)){
			echo $sql = "update sdb_mall_goods set brand_id='".$row['brand_id']."' where brand='".$row['brand_name']."'";
			echo "<br>";
			mysql_query($sql) or  err(__LINE__." ".mysql_error());
		}		
