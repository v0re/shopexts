...
function export_cat(){	
		...
		//迁移商品大类数据
		$sql = "insert into sdb_mall_offer_pcat (catid,offerid,pid,cat,catiffb) select id,1,0,classname,1  from yclass where ordername=''";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
	
		//迁移商品小类数据
		//1. 生成classname ->  id映射
		$sql = "select id,classname from yclass group by classname";
		$rs = mysql_query($sql);
		while($row = mysql_fetch_array($rs)){
			$aPcatid[$row['classname']] = $row['id']; 
		}
		//生成小分类
		$sql = "select id,classname,ordername from yclass where ordername<>''";
		$rs = mysql_query($sql);
		while($row = mysql_fetch_array($rs)){
			$pid = $aPcatid[$row['ordername']];
			$sql = "insert into sdb_mall_offer_pcat (catid,offerid,pid,cat,catiffb) values ('{$row[id]}','1','{$pid}','{$row[classname]}','1')";
			mysql_query($sql) or  err(__LINE__." ".mysql_error());
		}
		//统一处理catpath
...
function export_goods(){
		...
		//导商品数据
		$sql = "Insert into sdb_mall_goods(gid,offerid,catid,goods,brand_id,bn,storage,priceintro,basicprice,price,danwei,intro,memo,smallimgremote,bigimgremote,onsale,uptime) select id,1,classid,yname,0,number,num,0,0,0,'','',concat(shuomin,'<br>',tag),smallimg,bigimg,1,unix_timestamp(time) from ybook  order by time DESC";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
