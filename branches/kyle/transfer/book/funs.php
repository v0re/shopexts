...
function export_cat(){	
		...
		mysql_query($sql) or  err(__LINE__." ".mysql_error());		
		//迁移商品大类数据
		$sql = "insert into sdb_mall_offer_pcat (catid,offerid,pid,cat,catord,catiffb) select c_id,1,0,c_name,c_order,1 from jk_class where nsort='shop' group by c_id ";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
		//迁移商品小类数据
		$sql = "insert into sdb_mall_offer_pcat (catid,offerid,pid,cat,catord,catiffb) select s_id,1,c_id,s_name,s_order,1 from jk_sort where c_id in (	 	29,30,31,32,33,34,35,36,38,55) group by s_id";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
		//统一处理catpath
...
function export_goods(){
		...
		mysql_query($sql) or  err(__LINE__." ".mysql_error());	
		//导商品数据
		$sql = "Insert into sdb_mall_goods(gid,offerid,catid,bn,goods,brand_id,storage,priceintro,price,intro,memo,smallimgremote,bigimgremote,onsale,uptime) select id,1,s_id,serial,name,0,99,price,price*discount*0.01,keyes,concat(remark_1,remark_2),smallimg,bigimg,1,tim from shop_product order by tim ASC";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());		
...
function export_users(){
		...
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
		//处理用户
		$sql = "Insert into sdb_mall_member(userid,offerid,user,password,name,sex,birthday,regtime,email) select id,1,username,password,username,sex,birthday,tim,email from user_data";
		mysql_query($sql) or  err(__LINE__." ".mysql_error());
		//将时间改回数字类型
