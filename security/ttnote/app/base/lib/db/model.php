...
class base_db_model implements base_interface_model{
    ...
    public function getList($cols='*', $filter=array(), $offset=0, $limit=-1, $orderby=null){
        $rows = $this->db->selectLimit('select '.$cols.' from '.$this->table_name(1)
            .' where '.$this->filter($filter),$limit,$offset);
                ...
                foreach( $rows as $key => $row ){
                    foreach( $need_columns as $column ){
                        switch(trim($columns[$column]['type'])){
                            case 'serialize':
                                if(($val=unserialize($row[$column])) !== false){
                                    $rows[$key][$column] = $val;
                                }
                            default:
                        }
    ...
    function instance($id,$cols='*'){
        $rows = $this->db->select('select '.$cols.' from '.$this->table_name(1)
    ...
    public function insert(&$data){
        ...
        for($i=0;$i<$col_count;$i++) {
            ...
            if(!isset($p['default']) && $p['required'] && $p['extra']!='auto_increment'){
                if(!isset($data[$k])){
                    trigger_error(($p['label']?$p['label']:$k).__('不能为空！'),E_USER_ERROR);
                }
            }
