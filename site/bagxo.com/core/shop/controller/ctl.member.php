...
//            $this->pagedata['delivery'] = $shipping->checkDlTypePay($this->pagedata['order']['shipping']['id'], $this->pagedata['order']['shipping']['area']);
    ...
    function saveSecurity(){
        ...
        $this->end($oMem->saveSecurity($this->member['member_id'],$_POST),'Password change successful');
...
/*         $this->end(true, '添加成功', $this->system->mkUrl('member','couponExchange'));         */
    ...
    function saveMember(){
        ...
        if($_POST['email']){
            if(!preg_match('/.+@.+$/',$_POST['email'])){
                $this->splash('failed', $this->system->mkUrl("member","setting"), __('The E-mail entered is invalid. Please modify'));
            }
        }
      if($_POST['birthday']){    
        ...
        if($oMem->save($this->member['member_id'],$_POST)){
          ...
          for($i=0;$i<count($allkeys);$i++){
            ...
            $this->splash('success', $url , __('Changes Successfully'));
        }else{
    ...
    function exchange($cpnsId) {
        ...
        $this->end(true, 'Exchange Complete', $this->system->mkUrl('member','couponExchange'));
    }
