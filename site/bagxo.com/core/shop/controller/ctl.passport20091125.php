...
class ctl_passport extends shopPage{
    ...
    function namecheck(){
        ...
        if($member->check_uname($name,$message)){
                echo '<div class="valisuccess">&nbsp;<strong>'.$name.'</strong>&nbsp;可以使用</div>';
        }else{
        ...
        if ($common){
            if($this->system->getConf('site.register_valide') == 'true' || $this->system->getConf('site.register_valide') == true){
                if($_COOKIE["S_RANDOM_CODE"]!=md5($_POST['signupverifycode'])){
                    $this->splash('failed','back','验证码录入错误，请重新输入');
                }
            }
            $passwdLen=strlen($_POST['passwd']);
            if($_POST['license']!='agree'){
                $this->splash('failed','back',__('同意注册条款后才能注册'));
            }elseif(!preg_match('/\S+@\S+/',$_POST['email'])){
                $this->splash('failed','back',__('邮件格式不正确'));
            }elseif($passwdLen<4){
                $this->splash('failed','back',__('密码长度不能小于4'));
            }elseif($passwdLen>20){
                $this->splash('failed','back',__('密码长度不能大于20'));
            } 
                        ...
                        if (!empty($message)){
                            ...
                            if ($info = $account->createUserFromPluin($_POST,$message,$uid)){
                                ...
                                if ($_POST['regType']=='buy'){
                                    ...
                                    $this->splash('success',$this->system->mkUrl('cart','checkout'),__('注册成功，并进入购买'),'',1);
                                }
                            }
                            else
                                $this->splash('failed','back',$message);
    ...
    function recover(){
        ...
        if(empty($this->pagedata['data']['member_id'])){
            $this->splash('failed','back','该用户不存在！');
        }
        $this->output();
    }
    ...
    function sendPSW(){
        ...
        if(($data['pw_answer']!=$_POST['pw_answer']) || ($data['email']!=$_POST['email'])){
            trigger_error('问题回答错误或当前账户的邮箱填写错误',E_USER_ERROR);
        }
        ...
        $this->end(true,'邮件已经发送',$this->system->mkUrl('passport','index'));
    }
    ...
    function logout(){    //退出
        ...
        if ($logoutinfo){
            ...
            $this->splash('success',$this->system->base_url().DEFAULT_INDEX,'成功退出，系统将返回首页');
        }
        else{
            $baseurl='http://'.$_SERVER['HTTP_HOST'].substr(PHP_SELF, 0, strrpos(PHP_SELF, '/') + 1);
    ...
    function verify(){
        ...
        if($this->system->getConf('site.login_valide') == 'true' || $this->system->getConf('site.login_valide') == true){
            if($_COOKIE["L_RANDOM_CODE"]!=md5($_POST['loginverifycode'])){
                $this->nowredirect('failed',base64_encode(str_replace(array('+','/','='),array('_',',','~'),$_POST['ref_url'])),'验证码录入错误，请重新输入');
            }
        }
        ...
        if ($obj=$pObj->function_judge('checkusername')){
            ...
            if ((is_array($uinfo)&&intval($uinfo[0])>0)||$info = $memberObj->verifyLogin($_POST['login'],$_POST['passwd'],$message)){
                ...
                $this->nowredirect('failed',base64_encode(str_replace(array('+','/','='),array('_',',','~'),$_POST['ref_url'])),'用户名或密码有误，请重新输入');
            }
        }
        $this->title = '';
    ...
    function getMsg(){
        return array(
            'login'=>array('succ'=>'登陆成功','fail'=>'登陆失败'),
            'logout'=>array('succ'=>'退出成功','fail'=>'退出失败')
        );
    }
    ...
    function checkusername($uname='',$passwd='',$forward='',$uid='',$email=''){ 
         ...
         if ($_COOKIE['CART_COUNT']){
            ...
            $this->splash('success',$this->system->mkUrl('cart','merge',array(1)).'?forward='.urlencode($url),
                    __('<h1>登录成功。系统发现您还保存着上次购物车中选购的商品。</h1>
                    <div style="border:1px solid #efefef;padding:6px; margin:5px;width:auto;line-height:22px; ">
                    1. <a class="lnk" href="'.$this->system->mkUrl('cart','merge',array(0)).'?forward='.urlencode($url).'">清空上次选购的商品，以本次为准</a>
                    <br>
                    2. <a  class="lnk" href="'.$this->system->mkUrl('cart','merge',array(1)).'?forward='.urlencode($url).'">将本次选购的商品合并进去</a>
                    <br>
                    3. <a  class="lnk" href="'.$this->system->mkUrl('cart','merge',array(2)).'?forward='.urlencode($url).'">继续保留上次选购的商品，放弃本次的</a></div>'),'',60);
         }else{
            $oCart->save('all', $cartCookie);
         }
         $this->splash('success',$url,__('登录成功'),'',1);
         exit;
         $this->_succ=true;
    }
}
