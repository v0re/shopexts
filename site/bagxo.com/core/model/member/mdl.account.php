...
class mdl_account extends shopObject {
    ...
    function check_uname($uname,&$message){
        ...
        $len = strlen($uname);
        if($len<3){
            $message = ', Sorry, your username must be between 3 and 20 characters long.';
            return false;
        }elseif($len>20){
            $message = ', Sorry, your username must be between 3 and 20 characters long.';
            return false;
        }elseif(!preg_match('/^([@\.]|[^\x00-\x2f^\x3a-\x40]){2,20}$/i', $uname)){
            $message = ', Sorry, only letters (a-z), numbers (0-9), underline (_),and periods (.) are allowed.';
            return false;
        }else{
    ...
    function check_email($email,&$message){
        if(!(eregi('^.+@.+$',$email))){
            $message = 'The email address is invalid, please modify.';
            return false;
        }else{
            return true;
        }
    ...
    function create($data,&$message){
        ...
        if(!$rs || $this->db->getRows($rs)){
            trigger_error('The user name is not available',E_USER_ERROR);
            return false;
        }
        ...
        if(empty($aData['passwd'])){
            ...
            if ($obj=$pObj->function_judge('edituser')){
                ...
                if ($res>0){
                    ...
                    trigger_error('The current password you gave is incorrect.', E_USER_ERROR);
                    return false;
                }
            }
            else{
            ...
            //                        $message = __('密码修改成功！');
                                    ...
                                    trigger_error('Password change failed', E_USER_ERROR);
                                    return false;
                                }

                            }else{
                                trigger_error('Sorry, your password must be between 4 and 20 characters long.', E_USER_ERROR);
                                return false;
                            }
                        }else{
                            trigger_error('Sorry, your password must be between 4 and 20 characters long.', E_USER_ERROR);
                        ...
                        trigger_error('The new passwords do not match. ', E_USER_ERROR);
                        return false;
                    }
                }else{
                    trigger_error('The current password you gave is incorrect.', E_USER_ERROR);
