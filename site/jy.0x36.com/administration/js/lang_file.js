...
function onApply(oCheckbox) {
    var sFilter = 'filter';
    var sUrl = ('' + document.location).replace(/[&]{0,1}filter=.*/, '');
	var bChecked = oCheckbox.checked == undefined ? oCheckbox.attr('checked') : oCheckbox.checked;
    if(bChecked && $('#adm-langs-look-for').val().length > 2)
        sUrl += (sUrl.indexOf('?') == -1 ? '?' : '&') + 'filter=' + $('#adm-langs-look-for').val();
