...
//				$tmp_url = $this->url."index.php?gOo=99bill_reply.do&";
			...
			case "ALIPAY_MASS": //支付宝批发客户
				...
				$hiddenString = "<input type=hidden name=shopex_encoding value=\"gb2312\">";
				$hiddenString.= "<input type=HIDDEN name=\"cmd\" value=\"0001\">"; //创建交易
				$hiddenString.= "<input type=HIDDEN name=\"subject\" value=\"".$subject."\">"; //商品名称(商店名+订单号)
				$hiddenString.= "<input type=HIDDEN name=\"body\" value=\"".$this->orderdetail."\">";  //描述(订单明细)
				$hiddenString.= "<input type=HIDDEN name=\"order_no\" value=\"".$this->orderno.$this->orderid."\">";  //商户订单号
				$hiddenString.= "<input type=HIDDEN name=\"type\" value=\"1\">";  //支付类型1：商品购买2：服务购买3：网络拍卖4：捐赠
				$hiddenString.= "<input type=HIDDEN name=\"price\" value=\"".$this->amount."\">";  //商品单价
				$hiddenString.= "<input type=HIDDEN name=\"transport\" value=\"3\">";  //批发客户则置成3
				$hiddenString.= "<input type=HIDDEN name=\"ordinary_fee\" value=\"".$this->ordinary_fee."\">";  //平邮费用
				$hiddenString.= "<input type=HIDDEN name=\"express_fee\" value=\"".$this->express_fee."\">";  //快递费用
				$hiddenString.= "<input type=HIDDEN name=\"number\" value=\"1\">";//购买数量
				$hiddenString.= "<input type=HIDDEN name=\"readonly\" value=\"true\">";//交易信息只读
				$hiddenString.= "<input type=HIDDEN name=\"buyer_name\" value=\"".$this->rname."\">";//买家姓名
				$hiddenString.= "<input type=HIDDEN name=\"buyer_address\" value=\"".$this->raddr."\">";//买家地址
				$hiddenString.= "<input type=HIDDEN name=\"partner\" value=\"2088002003028751\">";//合作伙伴
				$hiddenString.= "<input type=HIDDEN name=\"ac\" value=\"".$mac."\">";  //验证信息
			...
			case "ALIPAY_DIRECT": //支付宝直接支付网关
				...
				{
					$mac = md5($mac_direct); //对参数串进行私钥加密取得值
					$hiddenString = "<input type=hidden name=shopex_encoding value=\"gb2312\">";
					$hiddenString.= "<input type=HIDDEN name=\"cmd\" value=\"1001\">"; //创建交易
					$hiddenString.= "<input type=HIDDEN name=\"subject\" value=\"".$subject."\">"; //商品名称(商店名+订单号)
					$hiddenString.= "<input type=HIDDEN name=\"body\" value=\"".$this->orderdetail."\">";  //描述(订单明细)
					$hiddenString.= "<input type=HIDDEN name=\"order_no\" value=\"".$this->orderno.$this->orderid."\">";  //商户订单号
					$hiddenString.= "<input type=HIDDEN name=\"date\" value=\"".$date."\">";  //日期
					$hiddenString.= "<input type=HIDDEN name=\"price\" value=\"".$this->amount."\">";  //商品单价
					$hiddenString.= "<input type=HIDDEN name=\"type\" value=\"1\">";  //支付类型1：商品购买2：服务购买3：网络拍卖4：捐赠
					$hiddenString.= "<input type=HIDDEN name=\"transport\" value=\"3\">";  //3：虚拟物品
					$hiddenString.= "<input type=HIDDEN name=\"ordinary_fee\" value=\"".$this->ordinary_fee."\">";  //平邮费用
					$hiddenString.= "<input type=HIDDEN name=\"express_fee\" value=\"".$this->express_fee."\">";  //快递费用
					$hiddenString.= "<input type=HIDDEN name=\"number\" value=\"1\">";//购买数量
					$hiddenString.= "<input type=HIDDEN name=\"seller\" value=\"{$this->merid}\">";//交易信息只读
					$hiddenString.= "<input type=HIDDEN name=\"partner\" value=\"2088002003028751\">";//合作伙伴
					$hiddenString.= "<input type=HIDDEN name=\"ac\" value=\"".$mac."\">";  //验证信息
				...
				else
				{
					...
					$hiddenString.= "<input type=HIDDEN name=\"type\" value=\"1\">";  //支付类型1：商品购买2：服务购买3：网络拍卖4：捐赠
					$hiddenString.= "<input type=HIDDEN name=\"price\" value=\"".$this->amount."\">";  //商品单价
					$hiddenString.= "<input type=HIDDEN name=\"transport\" value=\"3\">";  //1：平邮2：快递3：虚拟物品
					$hiddenString.= "<input type=HIDDEN name=\"ordinary_fee\" value=\"".$this->ordinary_fee."\">";  //平邮费用
					$hiddenString.= "<input type=HIDDEN name=\"express_fee\" value=\"".$this->express_fee."\">";  //快递费用
					$hiddenString.= "<input type=HIDDEN name=\"number\" value=\"1\">";//购买数量
					$hiddenString.= "<input type=HIDDEN name=\"readonly\" value=\"true\">";//交易信息只读
