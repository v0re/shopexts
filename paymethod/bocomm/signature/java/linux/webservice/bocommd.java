...
class doComms implements Runnable {
	...
	public void run () {		
		try {
			...
			String op,message,signedmessage;
			...
			if((line = in.readLine()) != null && !line.equals(".") && line.length() > 5) {		
				...
				if(op.equals("veri")){
					String src =  args[1];
					String signed = args[2];
					int vcode = this.verify(src,signed);
					out.print(vcode);
				}
	...
	private String sign(String message) {
		...
		try
		{
			...
			int ret = client.initialize("ini/B2CMerchant.xml");			
			if (ret != 0) 
			{
	...
	private int verify(String src, String signed){
