...
class BasicSecurityToken extends SecurityToken {
  ...
  public function __construct($token, $maxAge, $owner, $viewer, $app, $domain, $appUrl, $moduleId) {
	// $iOwnerID = $iVisitorID = ((int)$_COOKIE['memberID'] > 0 ) ? (int)$_COOKIE['memberID'] : 0;
	$iOwnerID = $iVisitorID = 1;
    ...
    if (! empty($token)) {
	  ...
	  // $this->tokenData[$this->OWNER_KEY] = $iOwnerID;
      // $this->tokenData[$this->VIEWER_KEY] = $iVisitorID;
