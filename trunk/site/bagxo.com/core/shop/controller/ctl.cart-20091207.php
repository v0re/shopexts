...
class ctl_cart extends shopPage{
    ...
    function addGoodsToCart($gid=0, $pid=0, $stradj='', $pmtid=0, $num=1) {
        ...
        if($aParams['pid'] == -1){
            ...
            trigger_error('加入购物车失败：无此货品',E_USER_ERROR);
            $this->end();
        }
        if(!intval($num)) $num = 1;
        ...
        if($status === 'notify'){
            ...
            $this->setError(10001);
            trigger_error('加入购物车失败：商品缺货，转入缺货登记',E_USER_ERROR);
            $this->end();
        }elseif(!$status){
            $this->begin($_SERVER['HTTP_REFERER']);
            $this->setError(10002);
            trigger_error('加入购物车失败: 商品库存不足或者提交参数错误！',E_USER_ERROR);
            $this->end();
        }else{
            $this->redirect('cart');
        }
...
//                $this->setError('10000');
                trigger_error('有促销活动期间是不否允许使用优惠券',E_USER_ERROR);
            }
        }else{
            trigger_error('请输入优惠券',E_USER_ERROR);
        }

          $this->end(true, '成功加入购物车', $this->system->mkUrl('cart', 'index'));  
    }
