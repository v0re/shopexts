			...
			for($i=0;$i<$row;$i++)
			{
				...
				for($j=0;$j<$col;$j++)
				{
					...
					if($row = $this->dsql->GetArray("al",MYSQL_ASSOC))
					{
						...
						//if()
						$row['id'] =  $row['ID'];
						if($row['redirecturl']) $row['arcurl'] = $row['redirecturl']; else $row['arcurl'] = $this->GetArcUrl($row['id'],$row['typeid'],$row['senddate'],$row['title'],$row['ismake'],$row['arcrank'],$row['namerule'],$row['typedir'],$row['money']);
