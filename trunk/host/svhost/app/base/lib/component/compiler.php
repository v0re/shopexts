...
class base_component_compiler{
    ...
    function &compile($file_contents){
        ...
        foreach(preg_split('!'.$ldq.'(\s*(?:\/|)[a-z][a-z\_0-9]*|)(.*?)'.$rdq.'!is',$file_contents,-1,PREG_SPLIT_DELIM_CAPTURE) as $i=>$v){
            $i = $i%3;
            if($i==0){
    ...
    function post_compile(&$compiled_text){
        ...
        preg_match_all("/" . preg_quote('$this->__view_helper_model[\'') . "([^']*)" . preg_quote('\']->') . "/is", $compiled_text, $matchs);
        if(count($matchs[1])){
