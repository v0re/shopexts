...
class desktop_mdl_menus extends dbeav_model{
   ...
   function workgroup($permission_id){
       ...
       foreach($workground as $value){
          $aTmp = $this->getList('*',array('menu_type' => 'workground','workground' => $value));
   ...
   function adminpanel($aper){
            $admin_data = array();
            $panelgroups = $this->getList('*',array('menu_type' => 'panelgroup'));
            $aData = $this->getList('*',array('menu_type' => 'adminpanel','permission' => $aper, 'display' => 'true'));
