 ...
 *     $error_handler($rsa_obj);
 *
 *     // set error handler on Crypt_RSA object ( see dirname(__FILE__)/RSA/ErrorHandler.php for details )
 ...
 *     // check consistence of Crypt_RSA object
 *     $error_handler($rsa_obj);
 *     // set error handler ( see dirname(__FILE__)/RSA/ErrorHandler.php for details )
 *     $rsa_obj->setErrorHandler($error_handler);
 *     // sign many documents
...
class Crypt_RSA extends Crypt_RSA_ErrorHandler
{
     ...
     *        See contents of dirname(__FILE__)/RSA/Math folder for examples of wrappers.
